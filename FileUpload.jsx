...
const FileUpload = ({ onDataUpload, user }) => {
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      // البيانات تم حفظها بالفعل في الواجهة الخلفية
      // نحتاج فقط لإشعار المكون الرئيسي بالتحديث
      onDataUpload(analysisResults);
      
      setUploadStatus({ 
        type: 'success', 
        message: `تم رفع البيانات إلى المنصة بنجاح! تم حفظ إحصائيات ${analysisResults.citiesCount} مدن.` 
      });
      ...
      setUploadStatus({ type: 'error', message: 'حدث خطأ أثناء رفع البيانات' });
      setIsProcessing(false);
    }
  };
  ...
  const deleteFile = async (fileId) => {
    try {
      const response = await fetch(`/api/statistics/${fileId}`, {
        method: 'DELETE',
      });
      
      if (response.ok) {
        setUploadStatus({ type: 'success', message: 'تم حذف الملف بنجاح' });
        loadUploadedFiles();
      } else {
        setUploadStatus({ type: 'error', message: 'حدث خطأ أثناء حذف الملف' });
      }
    } catch (error) {
      setUploadStatus({ type: 'error', message: 'حدث خطأ أثناء حذف الملف' });
    }
