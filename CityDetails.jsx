...
import { ArrowRight, Download, Trash2, Calendar } from 'lucide-react';
...
const CityDetails = ({ city, onBack, statisticsData, userRole, cityDetails }) => {
  ...
  const handleDeleteStatistics = () => {
    if (confirm(`هل أنت متأكد من حذف إحصائيات ${city}؟`)) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex space-x-2 space-x-reverse">
          ...
          {(userRole === 'manager' || userRole === 'tech_admin' || userRole === 'digital_admin') && hasData && (
            <Button
              ...
              className="flex items-center space-x-2 space-x-reverse text-red-600 hover:text-red-700 hover:bg-red-50"
            >
      ...
      {!hasData ? (
        ...
        <>
          ...
          {/* New Detailed Opportunities Table */}
          {cityDetails && cityDetails.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>تفاصيل الفرص التطوعية</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b">
                        <th className="text-right p-3 font-semibold">رقم الفرصة</th>
                        <th className="text-right p-3 font-semibold">نوع الفرصة</th>
                    ...
                    <tbody>
                      {cityDetails.map((detail, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          <td className="p-3">{detail.opportunity_number}</td>
                          <td className="p-3">{detail.opportunity_type}</td>
                          <td className="p-3">{detail.city}</td>
                          <td className="p-3">{detail.start_date}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
